...
def vars_to_var(days,hours,mins,secs):
    d = str(days)
    h = str(hours)
    m = str(mins)
    s = str(secs)
...
def add_vars(org_date,dys,hrs,mins,secs):
    print(org_date)
...
#secs_to_add = vars_to_var(days,h,m,s)
datestamp = add_vars(datestamp, days, h, m, s)
datestamp = datestamp.strftime("%Y-%m-%dT%H:%M:%SZ")
